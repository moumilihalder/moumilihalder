# Hi, I'm Moumili Halder 👋

Final-year B.Tech CSE student | Full-Stack Developer

## Tech Stack

* **Frontend:** React.js, HTML, CSS, Tailwind CSS
* **Backend:** Node.js, Express.js
* **Database:** MongoDB
* **Tools:** Git, GitHub, Vercel

## Projects

### TalkNova

AI-powered conversational web application built with **React.js, Node.js, Express.js, and Google Gemini API**.
Includes real-time API communication and a responsive UI.

**Live Demo:** https://talk-nova-frontend.vercel.app

---

### NutriTrack

MERN-based **health & fitness tracker** featuring BMI calculator, diet planner, water tracker, and workout modules.
Includes **JWT authentication and MongoDB database integration**.

**Live Demo:** https://nutritrack-frontend.vercel.app

---

### NewsNexa

Responsive **news aggregator web application** built with **React.js and News API**.
Allows users to browse news by category and search keywords with a clean responsive UI.

**Live Demo:** https://news-nexa-frontend.vercel.com

---

⭐ Check out my repositories to explore the source code of these projects.

